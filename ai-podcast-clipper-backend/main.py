...
class ProcessVideoRequest(BaseModel):
    s3_key: str
    youtube_url: str | None = None  # If provided, download from YouTube first then upload to s3_key
...
def create_subtitles_with_ffmpeg(transcript_segments: list, clip_start: float, clip_end: float, clip_video_path: str, output_path: str, max_words: int = 5):
    ...
    # Watermark filter: small LUNARTECH.AI text burned into the upper-right safe area.
    # Uses Anton font (already installed), 36px (~3.3% of 1080px width), semi-transparent
    # white with a black shadow so it reads on any background.
    watermark_filter = (
        "drawtext="
        "text='LUNARTECH.AI':"
    ...
    ffmpeg_cmd = (
        ...
        f"-vf \"ass={subtitle_path},{watermark_filter}\" "
...
def download_youtube_to_s3(youtube_url: str, s3_key: str) -> None:
    """
    Download a YouTube video using yt-dlp, merge to MP4, and upload to S3.

    Prefers the best MP4 stream up to 1080p.  Falls back to any best format
    and re-muxes with ffmpeg so the output is always a single .mp4 file.
    """
    import tempfile

    print(f"[YouTube] Downloading {youtube_url} → S3 key {s3_key}")
    with tempfile.TemporaryDirectory() as tmpdir:
        output_template = os.path.join(tmpdir, "video.%(ext)s")

        cmd = [
            "yt-dlp",
            # Prefer best mp4 video + best m4a audio merged; fallback to best single file
        ...
        if result.returncode != 0:
            ...
            raise RuntimeError(f"yt-dlp failed with exit code {result.returncode}: {result.stderr[:500]}")
        ...
        if not candidates:
            raise FileNotFoundError(f"yt-dlp produced no output file in {tmpdir}")

        local_path = os.path.join(tmpdir, candidates[0])

        # If the file isn't already .mp4, re-mux with ffmpeg (no re-encode, fast)
        if not local_path.endswith(".mp4"):
            mp4_path = os.path.join(tmpdir, "video.mp4")
            subprocess.run(
                ["ffmpeg", "-y", "-i", local_path, "-c", "copy", mp4_path],
                check=True, capture_output=True,
            )
            local_path = mp4_path

        print(f"[YouTube] Upload {local_path} → s3://{os.environ['S3_BUCKET_NAME']}/{s3_key}")
        s3_client = boto3.client("s3")
        s3_client.upload_file(local_path, os.environ["S3_BUCKET_NAME"], s3_key)
