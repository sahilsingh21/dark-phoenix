  ...
  async ({ event, step }) => {
    ...
    try {
      const { userId, credits, s3Key, youtubeUrl } = await step.run(
        ...
        async () => {
          ...
          return {
            ...
            // null for direct-upload files; non-null for YouTube-ingested files
      ...
      if (credits > 0) {
        ...
        // Build the Modal request payload.
        // For YouTube-ingested files, include youtube_url so Modal downloads
        // the video to S3 at s3Key before running the processing pipeline.
        const modalPayload: { s3_key: string; youtube_url?: string } = {
          s3_key: s3Key,
        };
        if (youtubeUrl) {
